...
namespace Clothes_shop.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Name,ImageUrl,Price,Description,quantity,CategoryId,CreatedAt")] Products products, IFormFile ImageFile)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Name,Price,Description,quantity,ImageUrl,CategoryId,EditAt")] Products products, IFormFile? ImageFile)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    if (ImageFile != null && ImageFile.Length > 0)
                    {
                        ...
                        // 2. Gán đường dẫn mới (ghi đè đường dẫn cũ)
                        products.ImageUrl = "/images/products/" + fileName;
                    }
                    // Nếu ImageFile == null, products.ImageUrl vẫn giữ giá trị từ input hidden (ảnh cũ)
